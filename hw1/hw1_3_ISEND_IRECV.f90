...
contains
  ...
  subroutine update_edges_interior()
    ...
    if (any(left == my_id) .and. nid(3) > 0) then
       ...
       b3(2:n1me+1) = b(:,n2me)
       call mpi_isend(b3, n1me+2,mpi_double_precision,nid(3)-1,tag,mpi_comm_world,send_reqst,ierr)
       call mpi_irecv(b3e,n1me+2,mpi_double_precision,nid(3)-1,tag,mpi_comm_world,recv_reqst,ierr)
       call mpi_wait(recv_reqst,status,ierr)
       call update_E_edge()
       call mpi_wait(send_reqst,status,ierr)
